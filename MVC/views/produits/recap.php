...
?>
  ...
  qui lui même contiendra le détail de la commande si on demande son affichage -->
        ...
        <table class="table table-dark mb-1">
            <thead>
                ...
                <tr>
                    ...
                    <th class="text-center" colspan="6">Commande passée par: <?php $user = Utilisateur::findbyid(['id' => $commande['utilisateur_id']]);
    ...
    on pose la condition si notre tableau $details n'est pas vide et à nouveau si l'id de la ligne de commande du tableau est le même que le commande_id de nos achats présent dans $details -->
        <?php
        if (!empty($details) && $details[0]['commande_id'] == $commande['id']) : ?>
            ...
            <table class="table table-dark mt-0">
                ...
                <tbody>
                    <?php foreach ($details as $detail) :
                        // dans la table détail, nous avons accés  à la propriété produit_id ainsi pour obtenir le détail de notre produit 
                        //on appel la méthode find() du model produit.php à laquelle on transmet le produit_id de notre ligne de détail 
                        //en paramètre et accédons directement à la propriété souhaitée du produit */
